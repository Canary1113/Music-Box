...
namespace 音乐魔盒.Services
{
    public sealed class JianpuConverter
    {
        ...
        private static List<JianpuEvent> BuildTimelineWithRests(
            ...
            bool preferFlat)
        {
            var raw = new List<JianpuEvent>();
            foreach (var group in notes
                .GroupBy(n => Math.Max(0, n.StartTick))
                .OrderBy(g => g.Key))
            {
                var cluster = group.ToList();
                int start = Math.Max(0, group.Key);
                int duration = Math.Max(1, cluster.Max(GetVisualDurationTicks));
                int end = start + duration;

                var sounding = cluster
                    .Where(n => !n.IsRest)
                    .GroupBy(n => n.Midi)
                    .Select(g => g.First())
                    .OrderBy(n => n.Midi)
                ...
                if (sounding.Count == 0)
                {
                    raw.Add(new JianpuEvent(start, end, isRest: true));
                    continue;
                }

                var pitches = sounding
                    .Select(n =>
                    {
                ...
                raw.Add(new JianpuEvent(start, end, isRest: false, pitches));
            }
            ...
            int cursor = 0;
            foreach (var item in raw.OrderBy(e => e.StartTick))
            {
                if (item.StartTick > cursor)
                {
                    timeline.Add(new JianpuEvent(cursor, item.StartTick, isRest: true));
                }
